<?php
	...
	$sql = ("UPDATE `reported` SET `status` = 'disposed' WHERE rid = '$dec'");
	if (mysqli_query($conn, $sql)) {
		...
		if(mysqli_query($conn,$comm)){
			...
			if(mysqli_query($conn,$comm_new)){
				...
				if($count == 0){
					$recycle = ("INSERT INTO decomm( `decommid`, `elecid`,`qty`) VALUES(NULL,'$e_id','1')");
					...
					$recycle = ("UPDATE decomm SET qty = '$newres' WHERE 'elecid' = '$e_id'");
					if(mysqli_query($conn,$recycle)){
						header('Location: ../pages/issues.php?msg=1');
					}
					else{
